...
const run = (command, args, label) =>
  runCommand(command, args, label, {
    cwd: ROOT_DIR,
    env,
    logPrefix: `${buildProfileLabel}-build`,
  });
...
run(
  nodeCommand,
  [
    './tools/release/scan-desktop-runtime.mjs',
    'apps/desktop/web/dist',
    'apps/desktop/shell/gen/backend-runtime',
  ],
  `Scanning ${buildProfileLabel} desktop runtime for removed service systems`,
);
