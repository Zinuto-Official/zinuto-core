...
import type { FutuDataScopeBlockReason } from "../futu/futuSupportMatrix.js";
import { APP_LOCALE_BASE, isAppLocale, loadLocaleCatalog, type AppLocale } from "@zinuto/shared/i18n";

export type AppUiLanguage = AppLocale;
export const APP_UI_BASE_LANGUAGE: AppUiLanguage = APP_LOCALE_BASE;
...
const readUiConfigCatalogValue = (
  ...
  const resolvedLanguage = isAppLocale(language) ? language : APP_UI_BASE_LANGUAGE;
  const catalog = loadLocaleCatalog(resolvedLanguage, "uiConfig" as never) as Record<
    string,
    string
  >;
  const value = catalog[key];
  if (typeof value !== "string") {
    throw new Error("CUSTOM_INDICATOR_ENGINE_BUNDLE_MISSING");
  }
  return value;
};
